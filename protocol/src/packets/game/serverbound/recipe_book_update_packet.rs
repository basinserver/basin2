...
impl CodablePacket for RecipeBookUpdatePacket {
    fn encode(self, buf: &mut BytesMut) {
        ...
        match (self.purpose, self.data) {
            ...
            (RecipeBookUpdatePacketPurpose::Settings, RecipeBookUpdatePacketData::Settings(guiOpen, filteringCraftable, furnaceGuiOpen, furnaceFilteringCraftable, blastFurnaceGuiOpen, blastFurnaceFilteringCraftable, smokerGuiOpen, smokerFilteringCraftable)) => {
    ...
    fn decode(buf: &mut BytesMut) -> Result<Self> where Self: Sized {
        ...
        let data = match purpose {
            Shown => {
                RecipeBookUpdatePacketData::Shown(
                    buf.get_mc_string(32767)?
                )
            },
            Settings => {
