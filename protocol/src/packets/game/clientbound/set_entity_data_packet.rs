...
impl EntityMetadata {
    ...
    pub fn serialize(self, buf: &mut BytesMut) {
        ...
        match self {
            ...
            VillagerData { villager_type, villager_profession, level } => {
                buf.set_mc_u8(16);
    ...
    pub fn parse(buf: &mut BytesMut) -> Result<EntityMetadata> {
        ...
        Ok(match tag_type {
            ...
            1 => {
                Int(buf.get_mc_var_int()?)
            },
            2 => {
                Float(buf.get_mc_f32()?)
            },
            3 => {
                Str(buf.get_mc_string(32767)?)
            },
            4 => {
                Component(buf.get_mc_chat_component()?)
            },
            5 => {
                if buf.get_mc_bool()? {
            ...
            6 => {
                ItemStack(buf.get_mc_item_stack()?)
            },
            7 => {
                Boolean(buf.get_mc_bool()?)
            },
            8 => {
                Rotations {
                    x: buf.get_mc_f32()?,
            ...
            13 => {
                BlockState(buf.get_mc_var_int()?)
            },
            14 => {
                CompoundTag(buf.get_mc_nbt()?)
            },
            15 => {
                Particle(ParticleOptions::parse(buf.get_mc_enum()?, buf)?)
            },
            16 => {
                VillagerData {
